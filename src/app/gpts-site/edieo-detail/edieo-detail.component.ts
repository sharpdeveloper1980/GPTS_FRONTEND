...
export class EdieoDetailComponent implements OnInit {
  ...
  constructor(private headerService: HeaderService, private apiService:ApiService, private route: Router, private activatedRoute: ActivatedRoute) {
...
console.log(val);
  ...
  myFavEdieo(id, type) {
   this.apiService.myFavEdieo({'id':id, user_id:this.userid, type:type}).subscribe(val=>{ 
     // alert(val.msg);
     this.addFavMsg= val.msg;
      setTimeout(function(){
        $("#addToFavPopup").modal('show');
      },1000);
    })
 }

 removeBootstrapModal() {
  $("#addToFavPopup").modal('hide');  
 }
...
// addToFav(id, type, $event) {
//   let data = {
//     //"career_video_id":id, 'type':type, user_id:this.userid
//     "user_id": this.userId,
//       "career_video_id": id,
//       "type":type
//   }
...
// }
  ...
  //  alert(idStatic);
   ...
   if ((!this.loggedIn && idStatic=='edieoDetail18')|| (!this.loggedIn && idStatic=='edieoDetail19')||(!this.loggedIn && idStatic=='edieoDetail20')|| (!this.loggedIn && idStatic=='edieoDetail23')||(!this.loggedIn && idStatic=='edieoDetail24')){
   // alert('not login');
    $.each($("video"), function () {
      if ($(this).get(0).id === idStatic) {
        return;
